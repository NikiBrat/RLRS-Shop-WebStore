...
if (isset($_SESSION['Ime'])) 
{
  if ($_SESSION['Admin']==True) 
  {
    ...
    if(isset($_POST['dugme']))
    {
      ...
      $upit="delete from paketusluga where PaketID=".$paket;

      $rez = $mysqli->query($upit);

      if(!$rez)
      {
            $_SESSION['Ispis'] = "<script>alert('Ne možete obrisati paket dok neko od kupaca ima taj paket u korpi!')</script>";
        header("Location: dodajBundle.php");
      }

      else
      {
            $_SESSION['Ispis'] = "<script>alert('Uspešno brisanje artikla!')</script>";
        header("Location: dodajBundle.php");
      } 
    }
    ?>
...
?>
  ...
  <script type="text/javascript">
    ...
    function Obrisi(){
      var c = confirm('Da li ste sigurni da želite da obrišete Paket?');
      if(c == false){
